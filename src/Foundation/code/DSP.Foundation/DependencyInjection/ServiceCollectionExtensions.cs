...
namespace DSP.Foundation.DependencyInjection
{
    ...
    {
        private const string RenderingSuffix = "rendering";
        ...
        /// <summary>
        /// Adds all renderings in the current assembly, assuming a lifetime of Transient unless otherwise specified.
        /// </summary>
        public static void AddRenderingsInCurrentAssembly(this IServiceCollection serviceCollection, ServiceLifetime lifetime = ServiceLifetime.Transient)
        {
            AddTypesEndingWith(serviceCollection, RenderingSuffix, lifetime, Assembly.GetCallingAssembly());
        }

        /// <summary>
        /// Adds all renderings in the specified assembly, assuming a lifetime of Transient unless otherwise specified.
        /// </summary>
        public static void AddRenderings(this IServiceCollection serviceCollection, Assembly assembly, ServiceLifetime lifetime = ServiceLifetime.Transient)
        {
            AddTypesEndingWith(serviceCollection, RenderingSuffix, lifetime, assembly);
        }

        private static void AddTypesEndingWith(IServiceCollection serviceCollection, string endingWith, ServiceLifetime lifetime, params Assembly[] assemblies)
        {
            var allTypes = GetTypes(assemblies);

            var serviceTypes = allTypes.Where(ff => ff.Name.EndsWith(endingWith, StringComparison.OrdinalIgnoreCase)).ToList();
